...
> - `CoCaLoss.forward` adds the optional fused `caption_loss` argument before `output_dict`. Callers that previously supplied `output_dict` as the sixth positional argument must now pass `output_dict=...` by keyword.
