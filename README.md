...
> - **MaMMUT** — single text decoder used in two passes per [the paper](https://arxiv.org/abs/2303.16839): bi-directional without cross-attention for contrastive learning, causal with cross-attention for captioning (`mammut_*` configs; trains via the CoCa task/loss). Loads the released [LAION openMaMMUT-ViT-L-14](https://huggingface.co/laion/openMaMMUT-ViT-L-14-DataComp-1.4B-s12.8B-b180K) weights (pretrained tag `datacomp1b_s12_8b_b180k`, or directly via `hf-hub:` — fork-format configs and state dicts are translated on load). The `mammut_ViT-L-14` config reproduces the original LAION-fork numerics exactly via legacy flags (`pool_type="avg_all"`, `use_pad_mask=false`); new configs default to corrected masked-mean pooling and pad masking. A modern-arch decoder variant is available via `multimodal_cfg.text_arch="modern"` (`mammut-moderntext_*`)
