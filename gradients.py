...
def grad_cam_test(model,layer,image,text,targets,target_index,idx):
    requires_grad = {}
    ...
    with torch.cuda.amp.autocast(), torch.autograd.set_detect_anomaly(True), torch.set_grad_enabled(True), Hook(layer) as hook:
        ...
        text_probs[:,target_index].backward()
        #extract from layer 
        ...
        show_attention_map(grad_cam,image_name,layer_name=str(layer),caption=targets[target_index],write_to_disk=True,idx=idx)
...
from src.gradcam.utils import get_all_layers
layers = get_all_layers(model.visual,[])
#layers = model.visual.transformer.resblocks
print(len(layers))
#layer = model.visual.trunk.stages[-1]
#grad_cam_test(model,layer,image,text,targets,target_index,idx=0)

for idx,layer in enumerate(layers):
    try:
        grad_cam_test(model,layer,image,text,targets,target_index,idx=str(idx))
