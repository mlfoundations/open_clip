...
butterfly_root = "/local/scratch/cv_datasets/butterfly/clip_eval"
num_workers = 8
...
def make_classname(c):
    return f"Heliconius {c}"


def rich_description_template_wrapper(openai_template):
    def template(cls):
        s = openai_template(cls)
        s += " " + descriptions[cls]
        return s
...
def zero_shot_eval(model, data, args):
    ...
    for split in data.keys():
        ...
        classnames = [make_classname(c) for c in data[split].dataset.classes]
        if split == "scientific-names":
            classifier = zero_shot_classifier(
                model, classnames, openai_imagenet_template, args
            )
        elif split == "rich-descriptions":
            templates = [
                rich_description_template_wrapper(template)
...
if __name__ == "__main__":
    ...
    # initialize datasets
    data = {
        "rich-descriptions": get_dataloader(
            torchvision.datasets.ImageFolder(butterfly_root, transform=preprocess_val),
            batch_size=args.batch_size,
        ),
        "scientific-names": get_dataloader(
            torchvision.datasets.ImageFolder(butterfly_root, transform=preprocess_val),
            batch_size=args.batch_size,
        ),
    }
    ...
    descriptions = {}
    with open("data/butterflies/heliconius.json") as fd:
        for c, desc in json.load(fd)["data"].items():
            descriptions[make_classname(c)] = desc
