...
from .audio import CLIPAudioCfg, AudioTower
...
class CLAP(nn.Module):
    ...
    Structurally identical to CustomTextCLIP but with an audio tower
    instead of a vision tower. Returns 'image_features' key in output
    dict for compatibility with ClipLoss.
    """
    output_dict: torch.jit.Final[bool]
    ...
    def __init__(
        ...
        # training_head mode: projection is applied only during forward() (training),
        # not during encode_audio() (evaluation). Matches old Marianna fork pattern.
    ...
    def encode_audio(self, audio, normalize: bool = False):
        if self._training_head:
            # Return raw HTSAT features (no projection) — for evaluation
    ...
    def forward(
        ...
        if self._training_head and audio is not None:
            # Training path: get normalized raw features, then apply projection + re-normalize
        ...
        if self.output_dict:
            out_dict = {
                "image_features": audio_features,   # key matches ClipLoss param name
                "text_features": text_features,
                "logit_scale": self.logit_scale.exp(),
            }
            if self.logit_bias is not None:
