"""Pretrained models for Open-CLIP"""
...
def list_pretrained(as_str: bool = False):
    ...
    """
    return [":".join([k, t]) if as_str else (k, t) for k, v in _PRETRAINED.items() for t in v]
...
def download_pretrained_from_url(
    ...
    if os.path.isfile(download_target):
        if expected_sha256:
            with open(download_target, "rb") as f:
                if hashlib.sha256(f.read()).hexdigest().startswith(expected_sha256):
    ...
    with open(download_target, "rb") as f:
        if expected_sha256 and not hashlib.sha256(f.read()).hexdigest().startswith(expected_sha256):
...
def download_pretrained(
    ...
    """Download pretrained model from url or hf_hub"""
    target = ""
    if not cfg:
        return target
