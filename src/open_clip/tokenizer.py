...
class HFTokenizer:
    ...
    def __init__(
        ...
        self.special_tokens = {}  # special tokens for specific modes

        # Initialize tokenizer based on mode
        if self.tokenizer_mode == 'bert_clips':
            # FIXME to remove 'bert_clips' mode once 'clips' alternative verified
            self.special_tokens = {
                "bos_token": 1,
                "eos_token": 2,
                "cls_token": 101,
    ...
    def __call__(self, texts: Union[str, List[str]], context_length: Optional[int] = None) -> torch.Tensor:
        ...
        if self.tokenizer_mode == 'bert_clips':
            # FIXME to remove
            return self._bert_clips_tokenize(texts, context_length)
        elif self.tokenizer_mode == 'clips':
    ...
    def _bert_clips_tokenize(self, texts: List[str], context_length: int) -> torch.Tensor:
        """Custom tokenization for BERT-style tokenizers with special post-processing"""
        encoded = []
        for text in texts:
            encoding = self.tokenizer.encode(text, add_special_tokens=False)
            tokens = encoding.ids[:context_length - 3]  # Leave room for special tokens
        ...
        for i, tokens in enumerate(encoded):
            padded_tokens = self._pad_and_add_class_token(
                tokens,
                max_length=context_length,
                pad_token_id=self.special_tokens['pad_token'],
                cls_token_id=self.special_tokens['cls_token'],
            )
