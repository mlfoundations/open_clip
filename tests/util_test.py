...
def model_generate(model, preprocess_val, batches):
    y = []
    with torch.no_grad():
        for x in batches:
            x = torch.stack([preprocess_val(img) for img in x])
            y.append(
                [
                    open_clip.decode(i).split("<end_of_text>")[0].replace("<start_of_text>", "")
                    for i in model.generate(x)
                ]
            )
    return y
...
def forward_model(model, model_name, preprocess_val, image_batch, text_batch):
    ...
    if type(y[0]) == dict:
        ...
        for key in y[0].keys():
            if key != "logit_scale":
                out[key] = torch.cat([batch_out[key] for batch_out in y], dim=0)
            else:
        ...
        for i in range(len(y[0])):
            if i != 2:
                out.append(torch.cat([batch_out[i] for batch_out in y], dim=0))
            else:
...
def random_text_batch(batch_size, min_length=75, max_length=75):
    ...
    token_words = [
        x[1].replace("</w>", " ") for x in t.decoder.items() if x[0] not in t.all_special_ids
    ]
...
def create_random_text_data(path, min_length=75, max_length=75, batches=1, batch_size=1):
    text_batches = [random_text_batch(batch_size, min_length, max_length) for _ in range(batches)]
    print(f"{path}")
...
def get_data_dirs(make_dir=True, generative=False):
    data_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "data")
    if generative:
        input_dir = os.path.join(data_dir, "input_generative")
        output_dir = os.path.join(data_dir, "output_generative")
    else:
...
def create_test_data_for_model(
    ...
    input_dir, output_dir = get_data_dirs(generative=generative)
    if generative:
        output_file_text = os.path.join(output_dir, f"generative_{model_id}_random_text.pt")
        output_file_image = os.path.join(output_dir, f"generative_{model_id}_random_image.pt")
        output_file_logits = os.path.join(output_dir, f"generative_{model_id}_logits.pt")
    else:
    ...
    if generative:
        ...
        if overwrite or not text_exists:
            ...
            print(captions)
            torch.save(captions, output_file_text)
            logits = forward_model(model, model_name, preprocess_val, input_data_image, captions)[
                "logits"
            ]
            torch.save(logits, output_file_logits)
        return
    ...
    if overwrite or not text_exists:
        ...
        if create_missing_input_data and not os.path.exists(input_file_text):
            create_random_text_data(input_file_text, batches=batches, batch_size=batch_size)
    ...
    if overwrite or not image_exists:
        ...
        input_file_image = os.path.join(input_dir, f"random_image_{size[0]}_{size[1]}.pt")
        if create_missing_input_data and not os.path.exists(input_file_image):
            create_random_image_data(input_file_image, size, batches=batches, batch_size=batch_size)
        assert os.path.isfile(
            input_file_image
        ), f"missing input data, expected at {input_file_image}"
...
def create_test_data(models, batches=1, batch_size=1, overwrite=False, generative_models=None):
    models = list(
        set(models)
        .difference(
            {
    ...
    if generative_models is not None:
        print(f"generating test data for generative models:\n{generative_models}")
    ...
    return models, generative_models
...
def main(args):
    ...
    parser.add_argument("-a", "--all", action="store_true", help="create test data for all models")
    parser.add_argument(
        "--all_generative", action="store_true", help="create test data for all models"
    )
    parser.add_argument(
        "-m", "--model", type=str, default=[], nargs="+", help="model(s) to create test data for"
    )
    parser.add_argument(
        "-f",
        "--model_list",
    ...
    parser.add_argument(
        ...
        default=[],
        nargs="+",
        help="path to a text file containing a list of generative model names, one model per line",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-b", "--batch_size", default=1, type=int, help="test data batch size (default: 1)"
    )
    ...
    if args.generative_model_list is not None:
        with open(args.generative_model_list, "r") as f:
            generative_model_list = f.read().splitlines()
    if (
        not args.all
        and len(args.model + args.generative_model) < 1
        and len(model_list + generative_model_list) < 1
    ):
    ...
    generative_models = (
        open_clip.list_generative_models()
        if args.all_generative
        else args.generative_model + generative_model_list
    )
    print(generative_models)
    try:
        models, generative_models = create_test_data(
            models,
    ...
    if args.save_model_list is not None:
        ...
        with open(args.save_model_list, "w") as f:
            for m in models:
                print(m, file=f)
    if args.save_generative_model_list is not None and generative_models is not None:
        with open(args.save_generative_model_list, "w") as f:
