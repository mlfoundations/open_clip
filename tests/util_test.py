...
def random_text_batch(batch_size, min_length=75, max_length=75):
    ...
    token_words = [x[1].replace("</w>", " ") for x in t.decoder.items() if x[0] not in t.all_special_ids]
    # strings of randomly chosen tokens
    return ["".join(random.choices(token_words, k=random.randint(min_length, max_length))) for _ in range(batch_size)]
...
def create_test_data(models, batches=1, batch_size=1, overwrite=False):
    ...
    for model_name in models:
        print(model_name)
        create_test_data_for_model(model_name, batches=batches, batch_size=batch_size, overwrite=overwrite)
    return models
...
def main(args):
    ...
    parser.add_argument("-m", "--model", type=str, default=[], nargs="+", help="model(s) to create test data for")
    parser.add_argument(
        "-f", "--model_list", type=str, help="path to a text file containing a list of model names, one model per line"
    )
    parser.add_argument(
        "-s", "--save_model_list", type=str, help="path to save the list of models that data was generated for"
    )
    parser.add_argument("-g", "--git_revision", type=str, help="git revision to generate test data for")
    parser.add_argument("--overwrite", action="store_true", help="overwrite existing output data")
    parser.add_argument(
        "-n", "--num_batches", default=1, type=int, help="amount of data batches to create (default: 1)"
    )
    parser.add_argument("-b", "--batch_size", default=1, type=int, help="test data batch size (default: 1)")
    ...
    try:
        models = create_test_data(
            models, batches=args.num_batches, batch_size=args.batch_size, overwrite=args.overwrite
        )
    finally:
        if args.git_revision is not None:
